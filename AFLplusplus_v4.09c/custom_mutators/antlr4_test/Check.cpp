...
// void CheckDirectory(const std::string_view document_check_path,
//                     const std::string_view js_enginee_name) {
//   //
...
//     }
//     if (wat_file_num == 0) {
//       std::cout << js_enginee_name << "文件夹下没有.txt文件" << std::endl;
//       // return 0;
//     } else {
//       std::cout << js_enginee_name << "内txt文件个数：" << wat_file_num
//                 << std::endl;
//     }
//   }
// }

// void FileCopyAndRename(const std::string_view document_check_path,
//                        const std::string_view new_copy_path,
//                        const std::string_view js_enginee_name) {
//   // 批量进行分析，查看当前语法的问题
...
//           std::string(js_enginee_name) + "_" + file_name;
//       std::string new_file_path =
//           std::string(new_copy_path) + "/" + new_file_name;
//       std::string old_file_path =
//           std::string(document_check_path) + "/" + file_name;
//       std::filesystem::copy_file(old_file_path, new_file_path);
//       valid_file_count++;
//     }
//   }
//   fprintf(stdout, "%s valid_file_count: %d\n", js_enginee_name.data(),
//           valid_file_count);
// }
...
// void DocumentCreate(const std::string_view input_file_path,
...
//     std::string new_file_path = std::string(output_file_path) + "/" +
//     file_name; std::filesystem::create_directory(new_file_path);
//   }
// }
...
int main() {
  ...
  /*
    1.
    给定指定文件夹路径（检索路径是否存在，检查路径下是否存在文件，不存在路径或者文件不存在则报错）
    2.
    遍历文件夹下所有文件，获取所有文件名称，在获取的文件名称前添加父目录名称+‘-’名字构成新文件名称
    3. 将原文件拷贝到新文件中
    4. 用新名称替换拷贝后的文件名称
  */
  // 获取文件夹下所有.txt文件名称并统一添加指定前缀，建立各个引擎的poc名称库
  //   std::string test_check_path_all =
  //   "/home/xyf/antlr_wat/AFLplusplus_v4.09c/"
  //                                     "custom_mutators/antlr4_test/check";
  //   DocumentClear(test_check_path_all);
  //   //
  //   检查待拷贝的文件夹是否存在同时检查文件夹内.wat尾缀的文件个数，个数为0报错

  //   std::string document_check_path_parse_error =
  //       "/home/xyf/antlr_wat/AFLplusplus_v4.09c/custom_mutators/antlr4_test/"
  //       "input/"
  //       "check";
  //   CheckDirectory(document_check_path_parse_error, "check");
  //   FileCopyAndRename(document_check_path_parse_error, test_check_path_all,
  //                     "check");

  //   // 批量进行分析，查看当前语法的问题
  //   std::string output_file_path = "/home/xyf/antlr_wat/AFLplusplus_v4.09c/"
  //                                  "custom_mutators/antlr4_test/output";
  ...
  //   CustomStrVisitor *custom_wat_visitor;
  ...
  //         std::string(test_check_path_all) + "/" + file_name;
  //     std::string output_each_file_path =
  //         std::string(output_file_path) + "/" + dir_name;
  //     fprintf(stdout, "\n\ninput_file_path: %s\n", input_file_path.c_str());
  //     fprintf(stdout, "output_file_path: %s\n",
  //     output_each_file_path.c_str()); custom_wat_visitor =
  //         new CustomStrVisitor(input_file_path, output_each_file_path);
  ...
  //     for (auto &p :
  //     std::filesystem::directory_iterator(output_each_file_path)) {
  //       std::string file_name = p.path().filename().string();
  //       std::string file_path =
  //           std::string(output_each_file_path) + "/" + file_name;
